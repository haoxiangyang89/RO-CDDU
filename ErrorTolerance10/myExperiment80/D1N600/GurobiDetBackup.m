function [obj,p,detTime]=GurobiDet(myData,N,T)
    ...
    %CoonstraintBody = zeros(NumOfRow,NumOfCol);
    ...
    for t = 1:T
        ii=ii+1;
        CoonstraintBody(ii,t) = 1;
        for i = 1:N 
            CoonstraintBody(ii,T+(t-1)*N+i) = -myData.('HoldingCost')(t)+myData.('CommitmentCost')(i);
        end
    end
    ...
    for t = 1:T
        ...
        CoonstraintBody(ii,t) = 1;
        for i = 1:N 
            CoonstraintBody(ii,T+(t-1)*N+i) = myData.('ShortageCost')(t)+myData.('CommitmentCost')(i);
        end
    end
     %Y(t), p(i,t), w(i,t), v(i,t),  x(i,t)
    %pmin
    for i =1:N
        for t=1:T
            ii=ii+1;
            CoonstraintBody(ii,T+(t-1)*N+i)=1;
            CoonstraintBody(ii,T+3*N*T+(t-1)*N+i)=-myData.('pmin')(i);
        end
    end
    ...
    for i =1:N
        for t=1:T
            ii=ii+1;
            CoonstraintBody(ii,T+(t-1)*N+i)=-1;
            CoonstraintBody(ii,T+3*N*T+(t-1)*N+i)=myData.('pmax')(i);
        end
    end
    %Y(t), p(i,t), w(i,t), v(i,t),  x(i,t)
    %RD
    for i = 1:N
        for t= 1:T-1
            ii=ii+1;
            CoonstraintBody(ii,T+t*N+i)=1;
            CoonstraintBody(ii,T+(t-1)*N+i)=-1;
            CoonstraintBody(ii,T+3*N*T+(t-1)*N+i) = myData.('RD')(i);
        end
    end
    %Y(t), p(i,t), w(i,t), v(i,t),  x(i,t)
    %RU
    for i = 1:N
        ii=ii+1;
        CoonstraintBody(ii,T+i)=-1;
        CoonstraintBody(ii,T+3*N*T+i)=myData.('RU')(i);
        for t=2:T
            ii=ii+1;
            CoonstraintBody(ii,T+(t-1)*N+i)=-1;
            CoonstraintBody(ii,T+(t-2)*N+i)=1;
            CoonstraintBody(ii,T+3*N*T+(t-1)*N+i)=myData.('RU')(i);
        end
    end
    ...
    for i= 1:N
        ii=ii+1;
        CoonstraintBody(ii,T+i) = -1;
        CoonstraintBody(ii,T+N*T+i) = M;
        for t=2:T
            ii=ii+1;
            CoonstraintBody(ii,T+(t-1)*N+i)=-1;
            CoonstraintBody(ii,T+(t-2)*N+i)=1;
            CoonstraintBody(ii,T+N*T+(t-1)*N+i)=M;
        end
    end
    for i=1:N
        ii=ii+1;
        ...
        for tau = 2:min(myData.('Upupmin')(i),T)
            ii=ii+1;
            CoonstraintBody(ii,T+(tau-1)*N+i)=1;
            CoonstraintBody(ii,T+(tau-2)*N+i)=-1;
            CoonstraintBody(ii,T+N*T+i)=-M;
        end
        for t= 2: T 
            for tau = t:min(t+myData.('Upupmin')(i)-1,T)
    ...
    for i= 1:N
        ii=ii+1;
        CoonstraintBody(ii,T+i) = 1;
        CoonstraintBody(ii,T+2*N*T+i) = M;
        for t=2:T
            ii=ii+1;
            CoonstraintBody(ii,T+(t-1)*N+i)=1;
            CoonstraintBody(ii,T+(t-2)*N+i)=-1;
            CoonstraintBody(ii,T+2*N*T+(t-1)*N+i)=M;
        end
    end
    for i =1:N
        ii=ii+1;
